...
def scorer_domaine(url, nom_entreprise, ville, code_ape, html_content=None):
    ...
    # ── RÈGLE STRICTE : au moins 1 élément de validation ─────────────────────
    reject_site = elements_validation < 1

    if reject_site:
        raisons_neg.append(f"Rejeté : aucun élément de validation (nom/ville/SIREN/activité)")

    return score, raisons_pos, raisons_neg, reject_site


def valider_site_officiel(url, nom_entreprise, ville, code_ape):
    """
    ...
    # Scraper le contenu
    soup = scraper_intelligent(url, timeout=8)
...
def trouver_meilleur_site(nom_entreprise, ville, code_ape, sites_candidats):
    ...
    sites_candidats : liste de tuples (url, source)
    """
    meilleur_url = None
    meilleur_score = -999
    ...
    for url, source in sites_candidats:
        if not url:
            continue
        # Vérification rapide domaine exclu
        domain = re.sub(r"https?://", "", url).split("/")[0].lower()
        ...
        url_validee, score, confidence, raison = valider_site_officiel(url, nom_entreprise, ville, code_ape)
...
def est_tel_hebergeur(telephone):
    """Détecte si un téléphone appartient à un hébergeur."""
    if not telephone:
        return False
...
RESEAUX_SOCIAUX = {
    "facebook": {
        ...
        "exclus": ["facebook.com/sharer", "facebook.com/share", "facebook.com/login",
                   "facebook.com/home", "facebook.com/photo", "facebook.com/watch",
                   "facebook.com/pages/create"],
    },
    "instagram": {
        "patterns": ["instagram.com/"],
        "exclus": ["instagram.com/p/", "instagram.com/explore", "instagram.com/accounts"],
    },
    "linkedin": {
        "patterns": ["linkedin.com/company/", "linkedin.com/in/"],
        "exclus": ["linkedin.com/sharing", "linkedin.com/login"],
    },
    "twitter": {
        "patterns": ["twitter.com/", "x.com/"],
        "exclus": ["twitter.com/intent", "twitter.com/share", "x.com/intent",
